...
function phpimageeditor_add_editlink(pathToEditor, pathToPlugin, hostPath, editImageText, language, joomlaPath)
{
			...
			for(var i=0;i<trs.length;i++)
			{
				...
				else 
				{
					...
					for(var c=0;c<links.length;c++)
					{
						...
						{
							imageSrcDetailed = phpimageeditor_urlencode(links[c].href.replace(hostPath, joomlaPath));
		...
		else
		{
			...
			for(var i=0;i<e.length;i++)
			{
				if (e[i].className == imgClassName && e[i].innerHTML.indexOf('folderup_32.png') == -1 && e[i].innerHTML.indexOf('folder.png') == -1)
				{
					...
					if (images.length > 0)
					{
						imageSrc = phpimageeditor_urlencode(images[0].src.replace(hostPath, joomlaPath));
						...
						if (phpimageeditor_file_is_image(imageSrc)) {
                            ...
                            if (isJoomla3) {
                            	...
                            	e[i].innerHTML += '<a style="background-color:#F9F9F9;color: #0B55C4; background-position: 5px 0; background-image: url('+pathToPlugin+'lite/shared/images/edit.gif); background-repeat: no-repeat; padding-bottom: 4px; padding-left: 22px; display: block;" href="'+pathToEditor+imageSrc+'&language='+language+'" target="_blank">'+editImageText+'</a>';
						}
					}
				}
			}
		...
		if (foundManager && foundItem)
		{
			for(var i=0;i<itemElements.length;i++)
			{
				if (itemElements[i].className == itemClass)
				{
					...
					{
						imageSrcArticle = phpimageeditor_urlencode(imagesArticle[0].src.replace(hostPath, joomlaPath));
