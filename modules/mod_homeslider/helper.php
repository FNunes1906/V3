...
class modHomeEventHelper{
	...
	$dateformat = mysql_query("SELECT homeslidercat ,date_format FROM jos_pageglobal LIMIT 1") or die(mysql_error());
	...
	$Query = "SELECT c.id , pc.title AS parenttitle FROM jos_categories AS c LEFT JOIN jos_categories AS pc ON c.parent_id = pc.id LEFT JOIN jos_categories AS mc ON pc.parent_id = mc.id LEFT JOIN jos_categories AS gpc ON mc.parent_id = gpc.id WHERE c.section = 'com_jevents' AND (c.id =".$format["homeslidercat"]." OR pc.id =".$format["homeslidercat"]." OR mc.id =".$format["homeslidercat"]." OR gpc.id =".$format["homeslidercat"].") AND c.published=1";
