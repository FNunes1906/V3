...
<div class="section">
	<!--Code for Mobiscroll NEW date picker - Yogi START -->
		...
		<label for="test_default" class="ui-btn-hidden button" style="width: 40%;float: left;">Ver Eventos por d&#237;a</label>
	...
	<!--Code for Event Category drop down Yogi Start -->
		<form id="event_cat_form">
			<select name="category_id" onChange="redirecturlcat(this.value)" class="catdisp">
				<option value="0" class="catdispopt">Toda la categoría</option>
			...
			while($row = mysql_fetch_array($rec)){
				...
				<li>	
					...
					<h3>
						...
						echo $categoryname[$n]; ?>
						<ul class="btnList">
							<li><a class="button small" href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '',$rowlocdetail['phone'])?>">llamar</a</li>
							<?php
							$ua = strtolower($_SERVER['HTTP_USER_AGENT']);
								if(stripos($ua,'android') != true) { ?>
