...
<li>
	...
	?>
		...
		<p>
			...
			?>
			<strong>Adresa:</strong>&nbsp;&nbsp<a href="map.php?lat=<?php echo $lat2;?>&long=<?php echo $lon2;?>">;<?php echo $row['street'];?></a>
			<br />
			<?php } 
			else {
			?>
			<strong>Adresa:</strong>&nbsp;&nbsp<a href="javascript:linkClicked('APP30A:SHOWMAP:<?php echo $lat2;?>:<?php echo $lon2;?>')"><?php echo $row['street'];?></a>
			<?php } ?>
		</p>
		...
		<?php if ($row['url']!=''){ ?>
		<p>
			<strong>Internet stranica:</strong>&nbsp;&nbsp;
		...
		<?php } ?>
		<ul class="btnList">
			<li><a href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '',$row[phone])?>" class="button small">Nazovi</a></li>
			
			<?php
			$ua = strtolower($_SERVER['HTTP_USER_AGENT']);
			if(stripos($ua,'iphone') == true) { 
			?>
			<li><a href="javascript:linkClicked('APP30A:FBCHECKIN:<?php echo $row[geolat];?>:<?php echo $row[geolon];?>')" class="button small">Prijavite se</a></li>
			<?php } ?>
		</ul>
		<?php  } ?>
	</li> <!-- end place -->
