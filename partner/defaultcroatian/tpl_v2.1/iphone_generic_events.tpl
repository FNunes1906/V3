...
<div class="section">
	...
	<label for="test_default" class="ui-btn-hidden">Događanja u dan</label>
	...
	<!--Code for Event Category drop down Yogi Start -->
		<form id="event_cat_form" class="cls_event_cat_form" autocomplete="off">
			<select name="category_id" onChange="redirecturlcat(this.value)" class="event_cat_drop">
				<option value="<?php echo $_SESSION['generic_category'];?>"><?php echo strtoupper("Kategorije");?></option>
		...
		while($row = mysql_fetch_array($rec)){
			...
			<li>	
				...
				<h3>
					...
					echo $categoryname[$n]; ?>
					<ul class="btnList">
						<li><a class="button small" href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '',$rowlocdetail['phone'])?>">Nazovi</a</li>
						<?php
						$ua = strtolower($_SERVER['HTTP_USER_AGENT']);
							if(stripos($ua,'android') != true) { ?>
						...
						<li><a class="button small" href="events_details.php?eid=<?php echo $row['rp_id'];?>&d=<?php echo $dateValue[2];?>&m=<?php echo $dateValue[1];?>&Y=<?php echo $dateValue[0];?>&lat=<?php echo isset($lat1);?>&lon=<?php echo isset($lon1);?>">Vi&#353;e</a></li>
					</ul>
				</h3> 
			</li>
