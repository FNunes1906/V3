	...
	<div id="searchBar">
		...
		<?php	
			...
			<select name="d" onChange="redirecturl(this.value)" >
				...
				<option value="alp" <?php if (isset($_REQUEST['filter_loccat']) && $_REQUEST['filter_loccat'] == 'alp') {?> selected <?php }?>>Alphabétique</option>
		...
		<?php
		if(isset($_POST['search_rcd'])!="Recherche") { 
		...
		if(isset($_POST['search_rcd'])=="Recherche"){ 
			...
			}else if($_POST['search_rcd'] == "Recherche" && $filter_loccat!=0){
