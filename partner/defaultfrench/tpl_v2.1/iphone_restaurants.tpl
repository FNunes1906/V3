...
<?php	while($rowsub=mysql_fetch_array($recsub))
{
	...
	if(mysql_num_rows($reccount)){
		...
		else if($_POST['search_rcd']=="Recherche"){ $search_query1="select * from `jos_jev_locations` where loccat IN (".implode(',',$allCatIds).") AND published=1 and loccat=$filter_loccat and title like '%$searchdata%' or description like '%$searchdata%' ORDER BY title ASC LIMIT " .$start_at.','.$entries_per_page;}
