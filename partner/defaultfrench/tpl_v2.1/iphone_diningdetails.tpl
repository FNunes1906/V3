...
<li>
	...
	?>
		...
		<p>
			<strong>Adresse:</strong>&nbsp;<a href="javascript:linkClicked('APP30A:SHOWMAP:<?php echo $lat2;?>:<?php echo $lon2;?>')"><?php echo $row['street'];?></a>
		</p>
	
		
		<p>
		<strong>Téléphone:</strong>&nbsp;&nbsp;<a href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '',$row['phone'])?>"><?php echo $row['phone']?></a>
		</p>
		...
		<?php } ?>
		<ul class="btnList">
			<li><a href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '',$row['phone'])?>" class="button small">Appeller</a></li>
			
			<?php
			$ua = strtolower($_SERVER['HTTP_USER_AGENT']);
			if(stripos($ua,'iphone') == true) { 
			?>
			<li><a href="javascript:linkClicked('APP30A:FBCHECKIN:<?php echo $row['geolat']; ?>:<?php echo $row[geolon]; ?>')" class="button small">Ajouter un lieu</a></li>
			<?php } ?>
		</ul>
		<?php  } ?>
	</li> <!-- end place -->
