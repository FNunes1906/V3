...
<div id="main" role="main">
	<div id="searchBar">
		...
		if(isset($_POST['search_rcd'])=="Pesquisar") {$searchdata = addslashes($_POST['searchvalue']);
			?>
			<?php
		if((isset($filter_loccat)==0) || ($_REQUEST['filter_loccat']=='alp') && ($_POST['search_rcd']=="Pesquisar")) {$search_query1="select * from `jos_jev_locations` where loccat IN (".implode(',',$allCatIds).") AND published=1 and title like '%$searchdata%' ORDER BY title ASC LIMIT " .$start_at.','.$entries_per_page;}
		...
		else if($_POST['search_rcd']=="Pesquisar"){ $search_query1="select * from `jos_jev_locations` where loccat IN (".implode(',',$allCatIds).") AND published=1 and loccat=$filter_loccat and title like '%$searchdata%' ORDER BY title ASC LIMIT " .$start_at.','.$entries_per_page;}
