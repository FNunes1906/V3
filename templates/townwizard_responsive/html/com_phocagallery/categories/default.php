...
<script type="text/javascript" src="<?php echo $templateDir ?>/js/popup/jquery.min.js"></script>
<script type="text/javascript" src="<?php echo $templateDir ?>/js/popup/jquery-ui.min.js"></script>
...
if ( $this->params->def( 'show_page_title', 1 ) ) {
	echo '<h2 id="middleColumnHeader" class="componentheading'.$this->params->get( 'pageclass_sfx' ).'">'. $this->params->get('page_title').'</h2>';
}
// Image, description
...
if ($this->tmpl['displayimagecategories'] == 1) {	
	for ($i = 0; $i < $countCategories; $i++) {
		if ( $columns == 1 ) {
			echo ' <div class="photosContainer">';
		} else {
			$float = 0;
		...
		//echo '<tr>';		
		echo '<h3><a href="'.$this->categories[$i]->link.'" class="category'.$this->params->get( 'pageclass_sfx' ).'">'.$this->categories[$i]->title.'</a></h3>';
		//if ($this->categories[$i]->numlinks > 0) {echo '<span class="small">('.$this->categories[$i]->numlinks.')</span>';}
		echo '<span><a href="'.$this->categories[$i]->link.'" class="see">';
...
function showcatImages(){
				...
				if($k == 1 && $k == $j){
					echo '<li><a class="pirobox_gall last" href="'.$l.'" title="'.$row['filename'].'">';
				}elseif($k == 1){
					echo '<li><a class="pirobox_gall first" href="'.$l.'" title="'.$row['filename'].'">';
				}elseif(($k == $j && $k <= 3)){
					echo '<li><a class="pirobox_gall last" href="'.$l.'" title="'.$row['filename'].'">';
				}elseif(($k == $j && $k > 3) ){
					echo '<li><a style="display:none;" class="pirobox_gall last" href="'.$l.'" title="'.$row['filename'].'">';
				}elseif(($k == $j || $k > 3) ){
					echo '<li><a style="display:none;" class="pirobox_gall" href="'.$l.'" title="'.$row['filename'].'">';
				}else{
					echo '<li><a class="pirobox_gall" href="'.$l.'" title="'.$row['filename'].'">';
				}
