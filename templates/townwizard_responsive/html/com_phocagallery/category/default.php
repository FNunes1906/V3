...
if ($this->tmpl['showpagetitle'] != 0) {
	if ( $heading != '') {
	    echo '<h2 id="middleColumnHeader" class="componentheading'.$this->params->get( 'pageclass_sfx' ).'">'.$heading.'</h2>'. "\n";
	} 
}
// Image, description
...
/*---------------------------------*/
		...
		if ($value->type == 1) {
			if ($value->displayname == 1 || $value->displayname == 2) {
				echo '<div class="phocaname" style="font-size:'.$this->tmpl['fontsizename'].'px"><dd><h3>'
				.PhocaGalleryText::wordDelete($value->title, $this->tmpl['charlengthname'], '...').'</h3></dd></div>';
			}
		}
		// image
		if ($value->type == 2) {
